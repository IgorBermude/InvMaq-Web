...
def listar_historico(maquina_id: Optional[int] = None) -> List[Dict]:
    ...
    base_query = (
        "SELECT h.id, h.id_maquina, h.data, h.hora, h.tecnico, h.descricao, m.nome AS maquina "
...
def adicionar_historico(id_maquina: int, data: str, hora: str, tecnico: str, descricao: str) -> None:
    """Insere um novo item no histórico."""
    run_query(
        "INSERT INTO historico (id_maquina, data, hora, tecnico, descricao) VALUES (%s, %s, %s, %s, %s)",
        (id_maquina, data, hora, tecnico, descricao),
    )
...
def atualizar_historico(id_: int, data: str, hora: str, tecnico: str, descricao: str) -> None:
    """Atualiza os campos de um registro do histórico."""
    run_query(
        """
        UPDATE historico
        SET data = %s, hora = %s, tecnico = %s, descricao = %s
        WHERE id = %s
        """,
        (data, hora, tecnico, descricao, id_),
    )